...
static int YUV_blend_with_pic (int width, int height,
  ...
  if (width != 640 || height != 480) { 
    ...
    scale_2dimage(pic_y_old, 640, 480, pic_y, width, height);
    scale_2dimage(pic_u_old, 320, 240, pic_u, width / 2, height / 2);
    scale_2dimage(pic_v_old, 320, 240, pic_v, width / 2, height / 2);
    }
    //}}}
...
static int yuvgen_planar (int width, int height,
  ...
  YUV_blend_with_pic(width, height, Y_start, Y_pitch, U_start, U_pitch, V_start, V_pitch, fourcc, alpha);

  return 0;
  }
//}}}
//{{{
static int upload_surface (VADisplay va_dpy, VASurfaceID surface_id, int box_width, int row_shift, int field) {
  ...
  VAStatus va_status = vaDeriveImage(va_dpy, surface_id, &surface_image);
