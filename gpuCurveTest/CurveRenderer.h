...
public:
	...
	void ComputeCubic (float x0, float y0, float x1, float y1,
										 float x2, float y2, float x3, float y3, int recursiveType = -1);

	CurveType DetermineType (CVSystem::MyPoint v0, CVSystem::MyPoint v1,
													 CVSystem::MyPoint v2, CVSystem::MyPoint v3,
													 double& d0, double& d1, double& d2, double& d3);

	void Triangulation (double x0, double y0, double x1, double y1,
											double x2, double y2, double x3, double y3, std::vector<double> klm);

	void DrawTriangle (MyVertex v0, MyVertex v1, MyVertex v2);

	void DrawPlainTriangle (double x0, double y0, double x1, double y1, double x2, double y2);

	void DrawPlainTriangle (double x0, double y0, double x1, double y1, double x2, double y2, bool isInside);
